...
library(DT)
library(bslib)
install.packages("shinycssloadersshinycssloaders")
...
#two way tables region, state
project2_1_ |>
  drop_na(Region, State) |>
  group_by(Region, State) |>
  summarize(count = n())

#two way tables category, sub-category
project2_1_ |>
...
#two way tables region, category
project2_1_ |>
  drop_na(Region, `Category`) |>
  group_by(Region, `Category`) |>
  summarize(count = n())

#Stacked Bar Graph Region, Category
...
#Stacked Bar Graph State, Segment
ggplot(data = project2_1_ |>
...
#Stacked Bar Graph Segment, Category
ggplot(data = project2_1_ |>
...
#numerical table, grouped
  ...
  summarize(across(c( Sales, Quantity, Discount, Profit), 
    ...
    #Create a histogram-Distribution of Age by Region
...
ggplot(project2_1_ , aes(x = Sales, fill = Region)) +
  ...
  labs(
    ...
    #Create a desnsity plot of discount by segment
...
ggplot(project2_1_, aes(x = Discount, fill = Segment)) +
 ...
 #Create a density plot of quantity by category
...
ggplot(project2_1_, aes(x = Segment, y = Quantity)) +
