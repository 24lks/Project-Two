...
# Histogram grouped by chosen variable (fill)
output$num_hist <- renderPlot({
  req(subset_data(), input$num_first, input$num_group)
  data <- subset_data()
  num_var <- input$num_first
...
# Boxplot grouped by selected variable
output$num_box <- renderPlot({
  req(subset_data(), input$num_first, input$num_group)
  data <- subset_data()
  num_var <- input$num_first
  x_var <- ifelse(input$num_group == "Selected Regions", "Region", "Segment")
...
# Density plot grouped by selected variable
output$num_density <- renderPlot({
  req(subset_data(), input$num_first, input$num_group)
  data <- subset_data()
  num_var <- input$num_first
  fill_var <- ifelse(input$num_group == "Selected Regions", "Region", "Segment")
...
output$num_plot_ui <- renderUI({
  req(input$summary_type == "num",
      input$explore_options == "plot",
      !is.null(input$num_plot_type))
