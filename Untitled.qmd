...
server <- function(input, output, session) {
  observeEvent(input$cat_region, {
  # If "Select All" is checked, select all regions
  if ("Select All" %in% input$cat_region) {
    updateCheckboxGroupInput(session, "cat_region",
                             selected = c("Select All", "Central", "East", "South", "West"))
  } else {
    # If all others are selected manually, also include "Select All"
    all_regions <- c("Central", "East", "South", "West")
    if (all(all_regions %in% input$cat_region)) {
      updateCheckboxGroupInput(session, "cat_region",
                               selected = c("Select All", all_regions))
    }
  }
})
  
   observeEvent(input$cat_segment, {
  # If "Select All" is checked, select all segments
  if ("Select All" %in% input$cat_segment) {
    updateCheckboxGroupInput(session, "cat_segment",
                             selected = c("Select All", "Consumer", "Corporate", "Home Office"))
  } else {
    # If all others are selected manually, also include "Select All"
    all_segments <- c("Consumer", "Corporate", "Home Office")
    if (all(all_segments%in% input$cat_segment)) {
      updateCheckboxGroupInput(session, "cat_segment",
                               selected = c("Select All", all_segments))
    }
  }
})
...
# Apply for first numeric variable
render_numeric_plot(input$num_first, input$num_plot_type, "num_hist_first")
render_numeric_plot(input$num_first, input$num_plot_type, "num_box_first")
render_numeric_plot(input$num_first, input$num_plot_type, "num_density_first")

# Apply for second numeric variable
render_numeric_plot(input$num_second, input$num_plot_type, "num_hist_second")
render_numeric_plot(input$num_second, input$num_plot_type, "num_box_second")
render_numeric_plot(input$num_second, input$num_plot_type, "num_density_second")
